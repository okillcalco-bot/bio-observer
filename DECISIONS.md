...
- **動画内タグの評価順(2026-09-06 再レビュー対応)**:作成日時タグ(creation_time / com.apple.quicktime.creationdate)は format tags → 各 stream tags の探索順で**すべて**候補①として評価し、最初に解釈できた値を採用する。先頭のタグが不正(invalid / timezone_unknown)でも後続の有効なタグを見逃さない(以前は1件目で打ち切っていた)。候補記録では priority=1 の候補が探索順に複数並び、`order` で全体の評価順を保持する。

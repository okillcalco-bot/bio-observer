...
- 日付:2026-08-09/決定者:Claude Code(T-101)/Issue #7
- **コピーとハッシュの1パス化+二重照合**:原本を読み取りのみでコピーしながらSHA-256を計算し(1パス)、コピー先(`.part`)を再ハッシュして原本と照合、成功後に atomic rename(`os.replace`)で確定パスへ移動する。コピー破損は登録前に検知される。
- **失敗時のクリーンアップ保証**:登録は「probe→容量確認→コピー→照合→重複確認→DB挿入→rename→commit」の順で行い、例外時は rollback+一時/確定ファイル削除を必ず実行する。登録失敗で中途半端なDB行・ファイルは残らない(テストでrename失敗を注入して検証)。
...
- **排他的・原子的な確定**:`os.replace`(既存を上書きする)をやめ、同一ディレクトリ内の `os.link` による確定名の排他的作成+一時ファイル削除へ変更。既存があれば FileExistsError となり上書きは構造的に不可能。**限界**:ハードリンク非対応FS(exFAT等)では存在確認+`os.replace` へフォールバックし、その場合のみ確認と確定の間にTOCTOU窓が残る(NTFS/ext4等の想定環境ではリンク方式が使われる)。

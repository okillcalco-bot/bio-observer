...
| T-004 | DBスキーマ実装 | DATA_MODEL.md 準拠のSQLiteスキーマ+マイグレーション基盤(全20テーブル。D-9/D-10/D-11反映、正確座標カラムなし=D-12、追記専用はトリガーで担保=D-23)。先行成果品の分析に基づく補強(Track⇄クリップ多対多、positive/insurance区分、実時刻確実性、特徴量ハイブリッド=D-24/D-25)。T-005指摘対応済み(原データDELETE禁止・系譜整合・Review整合・時刻範囲CHECK・系譜IDイミュータブル化)。DBテスト27件 | Claude Code | T-005指摘対応済み・再レビュー待ち(Issue #4) |
| T-005 | スキーマレビュー | DATA_MODEL.md との突合、追記性・移行性の確認(T-004のPRレビューとして実施) | Codex | 未着手 |

...
| ID | タスク | 内容 | 担当 | 状態 |
|---|---|---|---|---|
| T-101 | メディア登録 | 動画・音声取込(media_registry)、ストリーミングSHA-256+二重登録防止、FFprobeメタデータ、一時ファイル→照合→atomic rename、失敗時クリーンアップ、既存資産の不可侵・排他的確定、撮影開始日時の確実性ポリシー、確定処理の完全ロールバック・排他的フォールバック(D-26)。テスト18件(MVP要件1-3) | Claude Code | 実装完了・Codexレビュー待ち(Issue #7、PR作成済み) |

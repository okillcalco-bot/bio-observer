...
| T-112 | 撮影開始時刻の根拠優先順位 | 共通タイムラインの基準時刻改善:①動画メタデータcreation_time→②Drive modifiedTime→③ローカルファイル時刻→④人による補正(自動はestimated、人のみconfirmed=D-26維持)。採用根拠をRegistrationResult・IngestEvent・status.jsonへ記録。E2E観察項目もdocsへ追加。テスト6件。**マージはT-102着手前** | Claude Code | 実装完了・Codexレビュー待ち(Issue #12、PR作成済み) |

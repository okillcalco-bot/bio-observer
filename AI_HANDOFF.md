...
- **自己レビュー第2周(2026-09-21)**:クラッシュ/再開/重複の精密注入と実 Google クライアント経路(実 discovery+HttpMock、実 RefreshError)で再現した問題を修正。(中)登録 commit 直後のクラッシュ後に「自分の資産の重複」として完了 → note で自資産を採用。一時DLファイルの孤児化3経路 → `_discard_tmp` を決着・完了・failed で呼ぶ。一時停止アップロードの部分ファイルを使い続けて永久に取込不能 → 再開時の鮮度確認で waiting へ戻す。トークン応答 HTML の RefreshError が auth 扱い → transient。PySocks プロキシ例外が permanent → transient、PySocks を drive extra に追加。(低)ゴミ箱移動の検知(failed)、初期化失敗の案内分岐、座標ガードの NFKC・区切り、文書(同一 File ID 差し替え・confirmations=1・originals 孤児掃除未実装)。テスト8件(10ケース)追加(全137件)。次周の重点:今回の修正部分(鮮度確認・_discard_tmp・SourceRemovedError)の再レビューと、`inspect-time`/`register_media` の T-112 経路の最終確認
...
2. レビュー(Codex または調査責任者):PR #13(優先順位・確実性ポリシー維持・TZ解釈・スキーマ無変更の確認)、PR #15(例外分類・再試行ポリシー・秘匿・入力検証・スキーマ無変更の確認)。Codexが来るまでは Claude Code の自己レビュー周回を継続(第2周まで完了。次周は第2周の修正部分の再レビュー)
...
- `pytest`:137件すべてパス(環境確認7件+DB27件+メディア登録32件+Drive取込44件+CLI27件。パラメータ化テストを個別に数える。drive extra 導入環境)。drive extra なしでは 131 passed / 6 skipped。pyflakes クリーン。合成メディア・一時DB・Fake Driveのみ(本番DB・実Driveへは書き込まない)
